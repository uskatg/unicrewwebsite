/* unicrew smart app banner — iPhone fallback for non-Safari browsers.
   ...
   reappears on every page view (dismissing only hides it for the current
   page). */
(function () {
  var ua = navigator.userAgent;
  if (!/iPhone|iPod/.test(ua)) return;
  if (window.navigator.standalone) return; // already running as installed web app
  // Real Safari renders the native banner — skip. Other iPhone browsers carry
  // their own token (CriOS, FxiOS, …) or lack Safari's Version/ marker.
  var isSafari = /Version\/[\d.]+.*Safari/.test(ua) &&
